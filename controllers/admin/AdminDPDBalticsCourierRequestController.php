...
require_once __DIR__ . '/../../src/Entity/DPDCourierRequest.php';
...
class AdminDPDBalticsCourierRequestController extends AbstractAdminController
{
    ...
    private function initForm()
    {
        ...
        $phonePrefixRepository = $this->module->getContainer(PhonePrefixRepository::class);

        /** @var PhoneInputBuilder $phoneInputBuilder */
        $phoneInputBuilder = $this->module->getContainer(PhoneInputBuilder::class);
        ...
        if (Tools::getIsset('submitAdddpd_courier_request')) {
            ...
            $courierRequestRepository = $this->module->getContainer(CourierRequestRepository::class);
    ...
    private function renderPrefillSelect($prefix)
    {
        ...
        $addressRepository = $this->module->getContainer(AddressRepository::class);
    ...
    public function postProcess()
    {
        if (Tools::isSubmit('submitAdddpd_courier_request')) {
            ...
            $courierRequestValidator = $this->module->getContainer(CourierRequestValidator::class);
            ...
            if (!$courierRequestValidator->validate($courierRequestObj, $countryIso)) {
                $this->errors[] = sprintf(
                    $this->l('You can\'t create courier request because minimal time interval has to be at least %s'),
            ...
            $courierRequestService = $this->module->getContainer(CourierRequestService::class);
            try {
